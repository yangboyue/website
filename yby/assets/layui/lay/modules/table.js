...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function (exports) {
  ...
  var $ = layui.$
    ...
    , thisTable = function () {
      ...
      return {
        ...
        , refresh: function () { //杨博越添加用于只刷新当前分页数据，不重新渲染整个表格
          that.refresh.call(that);
        }
        , reloadExt: function (options) { //杨博越添加区别于原reload，不重新渲染整个表格
          that.reloadExt.call(that, options);
        }
        , reloadExtPage: function (options) { //杨博越添加区别于原reload，不重新渲染整个表格
          that.reloadExtPage.call(that, options);
        }
      }
    }
  ...
  //杨博越：表格完整重载
  Class.prototype.reloadExtPage = function (options, pages) {
    var that = this;
    if (that.config.data && that.config.data.constructor === Array) delete that.config.data;
    that.config = $.extend({}, that.config, options);
    that.page = pages;
  ...
  Class.prototype.renderData = function (res, curr, count, sort) {
    ...
    if (data.length === 0) {
      ...
      that.layMain.find('tbody').html('');
      that.layHeader.css('overflow', 'auto');//杨博越添加用于修复无数据时表头没有滚动条导致显示不全
      that.layMain.find('.' + NONE).remove();
  ...
  //杨博越：表格完整重载2
  table.reloadExtPage = function (id, options, page) {
    if (id) {
      var config = getThisTableConfig(id); //获取当前实例配置项
      if (!config) return;

      thisTable.that[id].reloadExtPage(options, page);
    }
  };
